...
<div x-data='{ childs : [], term_id: null,value:null, selected: <?php echo json_encode( $product_attributes ) ?? '[]' ?>,terms: <?php echo json_encode( $attributes ) ?> }'>
	<div class="fs-add-attributes" data-fs-element="item">
		...
		<select x-model="value" x-show="childs.length>0">
			...
			<template x-for="child in childs" x-key="'select-'+child.term_id">
	...
	<div x-text="selected.length"></div>
	<div class="fs-selected-attributes">
		<template x-for="(sChild,index) in selected" :key="'child-'+sChild.term_id">
			<div class="fs-selected-attribute" x-data="{name:sChild.name,value:sChild.term_id,parent:terms.find(item=>item.term_id===sChild.parent)}">
				<div x-text="terms.find(item=>item.term_id===sChild.parent).name" class="fs-selected-attribute__group"></div>
				<div x-text="sChild.name"></div>
				<button x-on:click.prevent="selected.splice(index,1)">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30" class="fs-selected-attribute__remove"
