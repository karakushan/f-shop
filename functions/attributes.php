...
function fs_current_screen_attributes( $group_id = 0, $args = array() ) {
	...
	if ( is_tax() ) {
		...
		$post_args ['tax_query']      = isset( $wp_query->tax_query->queries )
...
function fs_attr_filter( $group_id, $args = array() ) {
	...
	if ( $args['current_screen'] ) {
		$terms = fs_current_screen_attributes( $group_id, [ 'include_query' => $args['include_query'] ] );
	} else {
		if ( ! $args['childs'] ) {
...
//			do_action( 'qm/debug', $wp_query->query_vars );
			...
			$value = apply_filters( 'fs_attr_filter_value', $value, $term );
			...
			$remove_attr = apply_filters( 'fs_attr_filter_remove_attr', $remove_attr, $term );

			if ( ! $args['redirect'] ) {
				$value = $term->term_id;
			}

			$attr_checked = apply_filters( 'fs_attr_filter_checked_attr', checked( $term->term_id, $product_attributes, 0 ), $term );


			do_action( 'fs_attr_filter_item_before', [
				'term'        => $term,
				'value'       => $value,
				'remove_attr' => $remove_attr
			] );

			echo '<input type="checkbox" class="' . esc_attr( $args['input_class'] ) . '" data-fs-action="filter" data-fs-redirect="' . esc_url( $remove_attr ) . '" name="attributes[' . esc_attr( $term->slug ) . ']" value="' . esc_attr( $value ) . '"  ' . $attr_checked . ' id="check-' . esc_attr( $term->slug ) . '">';
			...
			echo '<label for="check-' . esc_attr( $term->slug ) . '"  class="' . esc_attr( $args['label_class'] ) . '">' . $label_before_text . ' ' . $term->name . ' <i>' . $count . '</i></label >';
			if ( $args['childs'] ) {
				fs_attr_filter( $term->term_id, $args );
			}

			do_action( 'fs_attr_filter_item_after', [
				'term'        => $term,
				'value'       => $value,
				'remove_attr' => $remove_attr
			] );
