...
class FS_Filters
{
    ...
    public function __construct()
    {
        ...
        add_action('pre_get_posts', [$this, 'filter_products_admin'], 10, 1);

        // Filter by product categories in the admin panel
        add_action('restrict_manage_posts', [$this, 'category_filter_admin']);

        // Setting the quantity of goods on the page of the archive of goods
        add_action('template_redirect', [$this, 'redirect_per_page']);

        // Admin redirects
        add_action('admin_init', [$this, 'redirects_admin_pages']);

        // Display the product edit fields in the quick editing mode
        add_action('quick_edit_custom_box', [$this, 'product_quick_edit_fields'], 10, 2);

        // Save custom fields in quick edit mode
        add_action('save_post', [$this, 'save_quick_edit_fields'], 10, 3);

        // Alternative: Direct AJAX handler for inline-save (higher priority)
        add_action('wp_ajax_inline-save', [$this, 'save_quick_edit_fields_direct'], 1);

        // Modify the saved product field
        add_filter('fs_filter_meta_field', [$this, 'fs_filter_meta_field'], 10, 3);
         ...
         */
        add_filter('fs_filter_meta_field', [$this, 'transliteration_product_slug'], 10, 3);
    }
     ...
     * @return mixed|string
     */
    public function transliteration_product_slug($title, $field_name, $post)
    {
     ...
     * @return float
     */
    public function fs_filter_meta_field($value, $field_name, $post)
    {
        if ($field_name == FS_Config::get_meta('price')) {
            $value = floatval(str_replace([','], ['.'], sanitize_text_field($value)));
        }
     ...
     * Display the product edit fields in the quick editing mode.
     */
    public function product_quick_edit_fields($column_name, $post_type)
    {
    ...
    /**
     * Saves custom product fields when using Quick Edit
     * Сохраняет кастомные поля товара при использовании Quick Edit.
     */
    public function save_quick_edit_fields($post_id, $post, $update)
    {
        ...
        // Check if this is a Quick Edit request (AJAX request with inline-save action)
        if (!wp_doing_ajax() || !isset($_POST['action']) || $_POST['action'] !== 'inline-save') {
            return;
        }

        // Debug: Log what's in $_POST
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('F-Shop Quick Edit Debug - POST data: '.print_r($_POST, true));
        }

        // Save custom fields if they exist in the POST data
        $custom_fields = [
            'price' => FS_Config::get_meta('price'),
            'sku' => FS_Config::get_meta('sku'),
            'remaining_amount' => FS_Config::get_meta('remaining_amount'),
        ]; 

         foreach ($custom_fields as $field_type => $meta_key) {
             if (isset($_POST[$meta_key])) {
                 $value = sanitize_text_field($_POST[$meta_key]);

                 // For price field, ensure it's a valid number
                 if ($field_type === 'price') {
                     $value = floatval($value);
                 }

                 // For stock field, handle empty values by deleting the meta
                 if ($field_type === 'remaining_amount') {
                     // Check if value is truly empty (not 0)
                     if ($value === '' || $value === null || trim($value) === '') {
                         // If empty, delete the meta field from database
                         delete_post_meta($post_id, $meta_key);
                         
                         // Debug: Log the deletion
                         if (defined('WP_DEBUG') && WP_DEBUG) {
                             error_log("F-Shop Quick Edit - Deleted {$meta_key} for post {$post_id} (empty value)");
                         }
                 ...
                 if (defined('WP_DEBUG') && WP_DEBUG) {
                     error_log("F-Shop Quick Edit - Saved {$meta_key} = {$value} for post {$post_id}");
                 }
             }
         }
    }
    ...
    /**
     * Direct AJAX handler for inline-save to ensure custom fields are saved
     * Прямой AJAX обработчик для inline-save чтобы гарантировать сохранение кастомных полей.
     */
    public function save_quick_edit_fields_direct()
    {
        // Check if this is a product post type
        ...
        // Debug: Log what's in $_POST
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('F-Shop Direct AJAX Debug - POST data: '.print_r($_POST, true));
        }
             ...
             if (isset($_POST[$meta_key])) {
                 ...
                 if ($field_type === 'remaining_amount') {
                     ...
                     if ($value === '' || $value === null || trim($value) === '') {
                         ...
                         // Debug: Log the deletion
                         if (defined('WP_DEBUG') && WP_DEBUG) {
                             error_log("F-Shop Direct AJAX - Deleted {$meta_key} for post {$post_id} (empty value)");
                         }
                         continue; // Skip to next field
                     } else {
                         $value = intval($value);
                 ...
                 // Debug: Log the saved value
                 if (defined('WP_DEBUG') && WP_DEBUG) {
                     error_log("F-Shop Direct AJAX - Saved {$meta_key} = {$value} for post {$post_id}");
                 }
             }
         }
    }
    ...
    public function redirects_admin_pages()
    {
        global $pagenow;
        // этот хак
     ...
     */
    public function filter_products_admin($query)
    {
        global $pagenow, $wpdb;
     ...
     */
    public function category_filter_admin()
    {
        global $typenow;
    ...
    public function attr_group_filter($group, $type = 'option', $option_default = 'Выберите значение')
    {
        ...
        if ($type == 'option') {
            echo ' <select name="attributes" data-fs-action="filter"><option value="'.esc_attr(remove_query_arg(['attributes'])).'"> '.esc_html($option_default).'</option> ';
            foreach ($fs_atributes as $key => $att) {
                $redirect_url = add_query_arg([
                    'fs_filter' => wp_create_nonce('f-shop'),
                    'attributes' => [$att->slug => $att->term_id],
                ]);
     ...
     */
    public static function fs_types_sort_filter($attr = [])
    {
    ...
    public static function get_used_filters()
    {
        ...
        $attributes = !empty($attributes_ids) ? get_terms([
