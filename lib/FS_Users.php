...
class FS_Users
{
    ...
    public static function register_form($args = [])
    {
        ...
        if (is_user_logged_in()) {
            ...
            ob_start(); ?>
			<form method="post" class="fs-login-form" action="" x-ref="registerForm" x-data="{ errors: [], msg: '', success: false, loading: false }"
				x-on:submit.prevent="loading = true; Alpine.store('FS').register($event).then((r)=>{
