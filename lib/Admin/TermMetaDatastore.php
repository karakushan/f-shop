...
class TermMetaDatastore extends \Carbon_Fields\Datastore\Datastore
{
    ...
    public function load(Field $field)
    {
        ...
        $raw_value = get_term_meta($this->get_object_id(), $key, true);
    ...
    {
        global $wpdb;
        $meta_value = $wpdb->get_var($wpdb->prepare("SELECT meta_value FROM $wpdb->termmeta WHERE term_id = %d AND meta_key = %s", $this->get_object_id(), $key));
    ...
    public function save(Field $field)
    {
        ...
        if (function_exists('wpm_ml_array_to_string')) {
            ...
            update_term_meta($this->get_object_id(), $key, $new_value);
        } else {
            update_term_meta($this->get_object_id(), $key, $value);
        }
    }
