...
//                    ),
			...
			FS_Config::get_data( 'drop_taxonomy' ) => [
				'fs_drop_price_url'          => [
					'name'     => __( 'Путь к прайсу в формате YML', 'f-shop' ),
					'type'     => 'text',
					'required' => false,
					'help'     => ''
				],
				'fs_drop_price_period'       => [
					'name'     => __( 'Частота обработки прайса', 'f-shop' ),
					'type'     => 'select',
					'args'     => [
						'values' => [
							'1'  => __( 'Каждый час', 'f-shop' ),
							'3'  => __( 'Каждые 3 часа', 'f-shop' ),
							'4'  => __( 'Каждые 4 часа', 'f-shop' ),
							'6'  => __( 'Каждые 6 часов', 'f-shop' ),
							'12' => __( 'Каждые 12 часов', 'f-shop' ),
							'24' => __( 'Каждые сутки', 'f-shop' ),
						]
					],
					'required' => false,
					'help'     => ''
				],
				'fs_drop_include_categories' => [
					'name'     => __( 'Парсить только перечисленные категории', 'f-shop' ),
					'type'     => 'text',
					'required' => false,
					'help'     => 'укажите перечень категорий в формате: 1,2,3'
				],
			]
	...
	function shop_taxonomies() {
		$taxonomies = array(
			...
			FS_Config::get_data( 'drop_taxonomy' )           => array(
				'object_type'        => FS_Config::get_data( 'post_type' ),
				'label'              => __( 'Поставщики (дроп.)', 'f-shop' ),
				'labels'             => array(
					'name'          => __( 'Поставщики (дроп.)', 'f-shop' ),
					'singular_name' => __( 'Поставщик', 'f-shop' ),
					'add_new_item'  => __( 'Добавить поставщика', 'f-shop' ),
				),
				//					исключаем категории из лицевой части
				"public"             => false,
				"show_ui"            => true,
