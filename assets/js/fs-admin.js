...
jQuery(function ($) {
    ...
    $(document).on('input propertychange', '#fs-search-input', function (event) {
        var $this = $(this);
        var $delay = 500;
        ...
        $this.data('timer', setTimeout(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    let html = '<ul class="fs-livesearch-data">';
                    data.data.forEach(function (item, index) {
                        html += '<li><span>' + item.title + '</span><span><button class="button select-product" type="button" data-product=\'' + JSON.stringify(item) + '\'>Выбрать</button></span></li>'
                    })
                    html += '</ul>';

                    $("#fs-search-product-results").html(html);
    ...
    $(document).on('click', '#fs-search-product-results .select-product', function (event) {
        ...
        if (!checkIfExists) {
            $("#fs-search-product .selected")
                .fadeIn()
                .find('ul')
                .append('<li>' + product.title + '<button class="delete-item" data-index="' + selectedProducts.length + '">✕</button></li>');
    ...
    $(document).on('click', '#fs-search-product .selected .delete-item', function (event) {
        event.preventDefault()
    ...
    $(document).on('click', '#fs-search-product .selected .assign-order', function (event) {
        event.preventDefault()
        let html = '';
        selectedProducts.forEach(function (item, index) {
            let photo = item.photo ? item.photo : '/wp-content/plugins/f-shop/assets/img/image.svg'
            html += '<tr>' +
                '<td>' + item.ID + '<input type="hidden" name="fs_products[' + index + '][ID]" value="' + item.ID + '"></td>' +
                '<td class="order-items__image">' +
                '<a href="' + item.link + '" target="_blank"><img src="' + photo + '" alt="' + item.title + '"></a>' +
                '</td>' +
                '<td><a href="' + item.link + '" target="_blank"><strong>' + item.title + '</strong></a></td>' +
                '<td>' + item.sku + '</td>' +
                '<td>' + item.price + ' ' + item.currency + '</td>' +
                '<td><input type="number" min="1" step="1" size="3" name="fs_products[' + index + '][count]" value="1"></td>' +
                '<td></td>' +
                '<td>' + item.price + ' ' + item.currency + '</td>' +
                '<td>' +
                '   <button type="button" class="remove-from-cart"><span class="dashicons dashicons-trash"></span></button>' +
                '</td>'
            '</tr>';
        })
    ...
    $(document).on('click', '.order-items .remove-from-cart', function (event) {
        event.preventDefault()
